...
<section class="forms py-2">
    <div class="container-fluid px-2">
        <x-page-title>
            ...
            <x-slot name="buttons">
                <a style="color: white" href="{{action('CustomerTypeController@create')}}" class="btn btn-primary"><i
                    ...
                    <table id="store_table" class="table dataTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>@if(!empty($customer_type->customers))<a
                                        href="{{action('CustomerTypeController@show', $customer_type->id)}}?show=customers"
                                        class="btn">{{$customer_type->customers->count()}}</a>@endif</td>
                                <td><a href="{{action('CustomerTypeController@show', $customer_type->id)}}?show=discounts"
                                        class="btn">{{@num_format($customer_type->total_sp_discount +
                                        $customer_type->total_product_discount +
                                        $customer_type->total_coupon_discount)}}</a>
                                </td>
                                @php
                                ...
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default"
                                            ...
                                            <li class="divider"></li>
                                            @endcan
                                            @if($customer_type->name != 'Walk in' && $customer_type->name != 'Online
                                            customers')
                            ...
                            @php
                            $total_discounts += $customer_type->total_sp_discount +
                            $customer_type->total_product_discount +
