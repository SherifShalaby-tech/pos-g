...
<tr class="product_row">
    ...
    <td>
        ...
        <input type="text" class="form-control quantity quantity_{{$i}}" data-val="0" name="add_stock_lines[{{$i}}][quantity]" required min="{{ in_array(strtolower($product->units->pluck('name')[0]), ['piece', 'قطعة', 'قطعه', ''], true) ? 1 : 0.00001 }}"
            value="0"  index_id="{{$i}}">
    ...
    <td>
        ...
        <input type="text" class="form-control selling_price selling_price_{{$i}}" name="add_stock_lines[{{$i}}][selling_price]" required index_id="{{$i}}"
               value="@if(isset($sell_price)){{@num_format((float)$sell_price)}}@else @if($product->selling_price_depends == null) {{@num_format((float)$product->sell_price)}} @else {{@num_format((float)$product->selling_price_depends)}} @endif @endif"  >
{{--        <input class="final_cost" type="hidden" name="add_stock_lines[{{$i}}][final_cost]" value="@if(isset($product->default_purchase_price)){{@num_format($product->default_purchase_price / $exchange_rate)}}@else{{0}}@endif">--}}
    </td>
    <td>
