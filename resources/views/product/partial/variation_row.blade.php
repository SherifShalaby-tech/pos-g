...
<tr class="row_{{$row_id}} variation_row" data-row_id="{{$row_id}}">
    ...
    <td class="default_purchase_price_td @if(isset($is_service) && $is_service == 0) hide @endif ">{!! Form::text('variations['.$row_id.'][default_purchase_price]', $product_purchase_price , ['class' =>
        'form-control
        default_purchase_price']) !!}</td>
    <td class="default_sell_price_td @if(isset($is_service) && $is_service == 0) hide @endif ">{!! Form::text('variations['.$row_id.'][default_sell_price]', $product_sale_price,
