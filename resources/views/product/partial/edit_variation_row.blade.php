<tr class="{{$item->name == 'Default' ? "hide" : ''}} row_{{$row_id}}" data-row_id="{{$row_id}}">
    ...
    <td class="supplier_div @if(isset($is_service) && $is_service ==0) hide @endif">{!! Form::text('variations['.$row_id.'][default_purchase_price]', !empty($item) ?
        @num_format($item->default_purchase_price) :
        null, ['class' => 'form-control default_purchase_price']) !!}</td>
    <td class="supplier_div @if(isset($is_service) && $is_service ==0) hide @endif">{!! Form::text('variations['.$row_id.'][default_sell_price]', !empty($item) ? @num_format($item->default_sell_price) :
        null,
